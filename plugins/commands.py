...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            [
                InlineKeyboardButton("❆ Jᴏɪɴ Cʜᴀɴɴᴇʟ 1 ❆", url='https://t.me/+_N4GYhmbBcVmZGQ9')
            ],[
                InlineKeyboardButton("❆ Jᴏɪɴ Cʜᴀɴɴᴇʟ 2 ❆", url=invite_link.invite_link)
            ],[
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        if is_valid == True:
            madie = InlineKeyboardMarkup([
                [InlineKeyboardButton("Verify Successful, Get Your File", url=f"f"https://telegram.me/{temp.U_NAME}?start=files_{fileid}")],
            ])
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            filesarr.append(msg)
        k = await client.send_message(chat_id = message.from_user.id, text=f"<b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b>\n\nThis Movie Files/Videos will be deleted in <b><u>05 mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this ALL Files/Videos to your Saved Messages and Start Download there</i></b>")
    ...
    if not files_:
        ...
        try:
            ...
            title = '@KSeriesFilm  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@KSeriesFilm  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
...
async def shortlink(bot, message):
    ...
    chat_type = message.chat.type
    if chat_type == enums.ChatType.PRIVATE:
        return await message.reply_text(f"<b>Hey {message.from_user.mention}, This command only works on groups !\n\n<u>Follow These Steps to Connect Shortener:</u>\n\n1. Add Me in Your Group with Full Admin Rights\n\n2. After Adding in Grp, Set your Shortener\n\nSend this command in your group\n\n—> /shortlink ""{your_shortener_website_name} {your_shortener_api}\n\n#Sample:-\n/shortlink shareus.io xhMlwOvXzehIRx3SeCFS3ofgKk12\n\nThat's it!!! Enjoy Earning Money 💲\n\n[[[ Trusted Earning Site - https://shareus.io]]]\n\nIf you have any Doubts, Feel Free to Ask me - @Illegal_Developers\n\n(Puriyala na intha contact la message pannunga - @Illegal_Developers)</b>")
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete :(\n\nGive me a shortener website link and api along with the command !\n\nFormat: <code>/shortlink shareus.io xhMlwOvXzehIRx3SeCFS3ofgKk12</code></b>")
